...
if(videoNoC){
    videoC = JSON.parse(localStorage.getItem(config.project+'-video-'+videoNoC))
    if(!videoC)
        $.ajax({
            url: '/mumu/video?',
            ajaxCache:true,
...
function goNextVideo(){
    ...
    if(videoC){
        trueVideos.rows.unshift(videoC)
        trueVideos.currRows.unshift(videoC)

        exploreVideos.rows.unshift(videoC)
        exploreVideos.currRows.unshift(videoC)
        videoC=null
    }
